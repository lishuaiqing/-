...
def variable_summaries(name,var, with_max_min=False):
  with tf.name_scope(name):
    mean = tf.reduce_mean(var) # 计算均值
    tf.summary.scalar('mean', mean)
    with tf.name_scope('stddev'):
      stddev = tf.sqrt(tf.reduce_mean(tf.square(var - mean)))
    tf.summary.scalar('stddev', stddev) # 创建summary来观察损失值
...
class Actor(object):
    ...
    def __init__(self, config):
        ...
        self.old_softmax = tf.Variable(tf.ones([128]) , name='assign',dtype=tf.float32)
        ...
        #with tf.variable_scope('assign_op'):
            #self.assign_ops = []
            #for v_old, v in zip(self.old_softmax, self.log_softmax):
    ...
    def build_critic(self):
        ...
        with tf.variable_scope("critic"):
            ...
            self.critic.predict_rewards(self.encoder_output) #获得128 * 1维的变量
            variable_summaries('predictions',self.critic.predictions, with_max_min = True)  #pre 可视化
        ...
        with tf.name_scope('permutations'):
            ...
            for input_, path in zip(tf.unstack(self.input_,axis=0), tf.unstack(self.positions,axis=0)): # Unstack % batch axis
                self.ordered_input_.append(tf.gather_nd(input_,tf.expand_dims(path,1)))
                 #expand_dims 在path[1]处添加一个1  之后获取input_对应位的数组
            ...
            #将ordered_input变为矩阵，之后第三维和第一维转换
            # Tensor("permutations/transpose:0", shape=(2, 21, 128), dtype=float32)
            # 21个城市的x，y坐标顺序，  128组
            # [batch size, seq length +1 , features] to [features, seq length +1, batch_size]   Rq: +1 because end = start = first_city

            # Ordered coordinates  获取上一个城市与下一个城市的x    y坐标差
            # Ordered coordinates
    ...
    def build_optim(self):
        ...
        #将tf.GraphKeys.UPDATE_OPS制成列表
        #tf.GraphKeys.UPDATE_OPS  该函数保证其辖域中的操作必须要在该函数所传递的参数中的操作完成后再进行
        with tf.control_dependencies(update_ops):
            ...
            with tf.name_scope('reinforce'):
                ...
                self.loss1 = tf.reduce_mean(self.loss1,0)
                # Loss
            ...
            with tf.name_scope('state_value'):
                ...
                #self.loss2 = tf.reduce_mean(tf.square(self.reward - self.avg_baseline, self.critic.predictions))
